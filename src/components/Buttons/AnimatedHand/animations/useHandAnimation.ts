...
import {setUserPlay, toggleHelper} from '../../../../redux/actions/gameAction';
...
const useHandAnimation = ({onSwipe, move}: HandAnimationProps) => {
  const {userPlay} = useSelector(state => state.game.rockPaper.game);
  ...
  React.useEffect(() => {
    ...
    shadowRadius.value = active
      ...
      : 13;
  }, [active, shadowOpacity, shadowRadius]);
  ...
  }, [isHandEnabled, translateX, translateY]);
